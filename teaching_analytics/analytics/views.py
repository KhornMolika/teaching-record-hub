...
def download_records(request):
    ...
    if format_type == 'pdf':
        return generate_pdf_report(lecturer, sessions)
    elif format_type == 'xlsx':
        return generate_xlsx_report(lecturer, sessions)
    else:  # default to CSV
        return generate_csv_report(lecturer, sessions)


def generate_csv_report(lecturer, sessions):
    """Generate CSV report"""
    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = f'attachment; filename="teaching_records_{lecturer.lecturer_id}.csv"'
...
def generate_pdf_report(lecturer, sessions):
    ...
    # Title
    title = Paragraph("Teaching Records Report", title_style)
    ...
    response['Content-Disposition'] = f'attachment; filename="teaching_records_{lecturer.lecturer_id}.pdf"'
    response.write(pdf_data)
    
    return response


def generate_xlsx_report(lecturer, sessions):
    ...
    response['Content-Disposition'] = f'attachment; filename="teaching_records_{lecturer.lecturer_id}.xlsx"'
